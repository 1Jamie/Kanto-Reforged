...
local function loadRomEncounters()
  ...
  local paths = {
    home .. "/.local/share/love/pokemon-love2d/gold/data/generated/encounters.lua",
    "data/generated/encounters.lua",
  }
  for _, p in ipairs(paths) do
    local ok, rom = pcall(dofile, p)
    if ok and type(rom) == "table" and rom.grass and rom.grass.ROUTE_29 then
      return rom
    end
  end
  return nil
...
local function pokemonLookup(mod)
  local data = liveData(mod)
  local pack = package.loaded["mods.Kanto-Reforged.pokemon_data"]
...
function JohtoDex.snapshotVanillaNewOrder(mod)
  ...
  -- Headless / partial boots: pull ROM NewPokedexOrder from the Gold cache.
  local home = os.getenv("HOME") or ""
  local paths = {
    home .. "/.local/share/love/pokemon-love2d/gold/data/generated/pokedex.lua",
    "data/generated/pokedex.lua",
  }
  for _, p in ipairs(paths) do
    local ok, poke = pcall(dofile, p)
    if ok and type(poke) == "table" and poke.newOrder and #poke.newOrder >= 200 then
      vanillaNewOrder = {}
