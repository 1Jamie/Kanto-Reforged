# Kanto Reforged — battle sprite credits
...
- **base assets** — flat `assets/<SPECIES>_{front,back}.png` when that species is not in the gs pack
- **ROM** — vanilla game battle sprite (no custom art in this build yet)

Community artists are credited for gs-pack species (see `assets/g1-3 credit.csv`).
**1jamie / lady_gaia** are credited only for species that fall through to the flat base-assets collection.
